...
void uart_rs485_read(){
    ...
    // ESP_LOGI(TAG, "Starting RS485 read... TX %d bytes", tx_len);

    // Enable Transmit Mode and give DE time to assert
    //ESP_LOGI(TAG, "Setting RS485 to Transmit Mode");
    uart_rs485_set_transmit_mode();
    vTaskDelay(pdMS_TO_TICKS(5));

    // Send Inquiry Frame (binary safe)
    // ESP_LOGI(TAG, "Sending Modbus Request Frame...");
    // Print out modbus request frame in hex for debugging
    // ESP_LOGI(TAG, "Modbus Request Frame:");
    // for (int i = 0; i < tx_len; i++) {
    //     ESP_LOGI(TAG, "%02X ", tx_buf[i]);
    // }
    ...
    // ESP_LOGI(TAG, "Switching RS485 to Receive Mode");
    ...
    while (waited < total_wait) {
        ...
        if (res == ESP_OK && available > 0) {
            ...
            if (buf_len > sizeof(rx_buf)) buf_len = sizeof(rx_buf);
    ...
    if (computed != received) {
        ...
        // ESP_LOGI(TAG, "CRC Check Passed: 0x%04X", computed);
    }
